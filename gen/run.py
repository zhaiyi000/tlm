...
def run_tuning(for_type, finetuning_init, testtuning_init, ssh_target, model, target, init_times, finetuning_schedule_times, testtuning_schedule_times):
    ...
    if for_type == FOR_FINETUNING and finetuning_init:
        for test_file_idx in range(0, init_times, 1):
            ...
            command = f'''
                CUDA_VISIBLE_DEVICES={device_id_all} python gen_state.py \
                --model_name_or_path=gen_data/clm_gen_{model} \
                --sketch_path=gen_data/{model}_gen_train/0_merge.json \
                --save_path=gen_data/{model}_gen_train/gen_train.json \
                --allow_repeat=True \
                --target="{target}" \
                --keep_cnt=16
            '''
            ...
            command = f'rsync gen_data/{model}_gen_train/gen_train.json {ssh_target}:~/tlm/gen/measure_data/to_measure/finetuning_{test_file_idx}.json'
            exec_cmd_if_error_send_mail(command)
    ...
    if for_type == FOR_TESTTUNING and testtuning_init:
        for test_file_idx in range(0, init_times, 1):
            ...
            command = f'''
                CUDA_VISIBLE_DEVICES={device_id_all} python gen_state.py \
                --model_name_or_path=gen_data/clm_gen_best_{model} \
                --sketch_path=gen_data/{model}_gen_evaltuning/0_merge.json \
                --save_path=gen_data/{model}_gen_evaltuning/gen_eval.json \
                --allow_repeat=True \
                --target="{target}" \
                --keep_cnt=32
            '''
            ...
            command = f'rsync gen_data/{model}_gen_evaltuning/gen_eval.json {ssh_target}:~/tlm/gen/measure_data/to_measure/testtuning_{test_file_idx}.json'
            exec_cmd_if_error_send_mail(command)
    ...
    if True:
        ...
        while True:
            ...
            with lock:
                command = f'ssh {ssh_target} "cd ~/tlm/gen/measure_data/measured; ls -v"'
                ...
                if len(measured_files) > 0:
                    ...
                    if "finetuning_" in file_path:
                        command = f'rsync {ssh_target}:~/tlm/gen/measure_data/measured/{file_path} measured/{file_path}'
                        exec_cmd_if_error_send_mail(command)
                        ...
                        command = f'mv measured/{file_path} gen_data/measure_data_{model}/{file_path}'
                        exec_cmd_if_error_send_mail(command)

                        from utils import add_finetuning_files
                        add_finetuning_files(f"gen_data/measure_data_{model}/{file_path}")

                        exec_cmd_if_error_send_mail(f'python postprocess.py --target="{target}"')
                        ...
                        command = f'''
                            ...
                            --tokenizer_path=gen_data/gen_tokenizer_{model} \
                            --save_path=gen_data/{model}_gen_best
                        '''
                        exec_cmd_if_error_send_mail(command)
                        ...
                        if test_file_idx % 4 == 3:
                            command = f'''
                                CUDA_VISIBLE_DEVICES={device_id_all} python gen_state.py \
                                --model_name_or_path=gen_data/clm_gen_best_{model} \
                                --sketch_path=gen_data/{model}_gen_eval/0_merge.json \
                                --save_path=gen_data/{model}_gen_eval/gen_eval.json \
                                --allow_repeat=True \
                                --target="{target}" \
                                --keep_cnt=32
                            '''
                            ...
                            command = f'rsync gen_data/{model}_gen_eval/gen_eval.json {ssh_target}:~/tlm/gen/measure_data/to_measure/0_test_{test_file_idx}.json'
                            exec_cmd_if_error_send_mail(command)
                        ...
                        command = f'rsync gen_data/{model}_gen_train/gen_train.json {ssh_target}:~/tlm/gen/measure_data/to_measure/finetuning_{test_file_idx + init_times}.json'
                        exec_cmd_if_error_send_mail(command)

                        # exec_cmd_if_error_send_mail(f'rm clm_gen_best_{model}/*bin')

                        command = f'ssh {ssh_target} "cd tlm/gen/measure_data/measured; mv {file_path} ../moved/{file_path}"'
                        exec_cmd_if_error_send_mail(command)
                        ...
                        from utils import add_test_files
                        add_test_files(f"gen_data/measure_data_{model}/{file_path}")
                        
                        command = f'python speedup_eval.py --target="{target}" --for_test=True >> speedup_eval_{target_tvm.kind.name}.log 2>&1'
                        exec_cmd_if_error_send_mail(command)
                        ...
                        from utils import add_testtuning_files
                        add_testtuning_files(f"gen_data/measure_data_{model}/{file_path}")
                        ...
                        exec_cmd_if_error_send_mail(f'python task_sheduler.py --target="{target}" --for_testtuning=True')

                        keep_cnt_factor = 1
                        from task_sheduler import find_potential_files_len
                        len_tasks = find_potential_files_len()
                        if len_tasks == 0:
                        ...
                        command = f'rsync gen_data/{model}_gen_evaltuning/gen_eval.json {ssh_target}:~/tlm/gen/measure_data/to_measure/testtuning_{test_file_idx + init_times}.json'
                        exec_cmd_if_error_send_mail(command)

                        command = f'python speedup_eval.py --target="{target}" --for_testtuning=True >> speedup_eval_{target_tvm.kind.name}_testtuning.log 2>&1'
                        exec_cmd_if_error_send_mail(command)
...
def main():
    ...
    os.makedirs(f'gen_data/measure_data_{HARDWARE_PLATFORM}', exist_ok=True)
...
# PYTHONUNBUFFERED=1 python run.py --target="llvm -mcpu=core-avx2 -model=i7" --for_type=for_testtuning --testtuning_init=True |& tee run_i7.log
